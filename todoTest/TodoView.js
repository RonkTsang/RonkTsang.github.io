TodoView = function(){
  ...
  return {
    ...
    addTodo:function(todo){
      ...
      var list = $('#list');
      addFirstChild(list,todoLi);
      setTimeout(function(){todoLi.className = 'listItem';},50);
    ...
    holderLiToggle:function(){
      ...
      var holderLi = $('#holderLi');
      if((allLi.length-1-hiddenLi.length) > 0){
        addClass($('#holderLi'),'hidden');
      }else if((allLi.length-1-hiddenLi.length) == 0){
        return;
      }else{
        removeClass($('#holderLi'),'hidden');
      }
